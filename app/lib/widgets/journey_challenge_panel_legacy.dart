...
class _ChallengeSession {
  ...
  static _ChallengeSession _buildGoldParagraph({
    ...
    final correctTexts =
        source.skip(start).take(requiredCount).toList(growable: false);
  ...
  static _ChallengeSession _buildMissing(
    ...
    final correct =
        source.length > start + 1 ? source[start + 1] : '他沿着主要路线慢慢向前走。';
    final after =
        source.length > start + 2 ? source[start + 2] : '一路上的景色因此不断发生变化。';
  ...
  static List<String> _missingDistractors(
    ...
    final discoveryCandidate =
        discoverySentences.isEmpty ? null : discoverySentences.last;

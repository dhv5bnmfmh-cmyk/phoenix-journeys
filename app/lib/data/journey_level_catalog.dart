...
class JourneyLevelContent {
  ...
  JourneyLevelContent withReadingLimit({
    ...
    final storyRanges = _balancedRanges(storyParagraphs.length, safeParagraphCount);
    final discoveryRanges = _balancedRanges(discoveries.length, safeDiscoveryCount);

    return JourneyLevelContent(
      storyParagraphs: storyRanges
          .map((range) => _joinChinese(storyParagraphs.sublist(range.start, range.end)))
          .toList(growable: false),
      storyAnnotations: storyRanges
          .map((range) => _mergeAnnotations(storyAnnotations.sublist(
      ...
      discoveries: discoveryRanges
          .map((range) => _mergeDiscoveries(discoveries.sublist(range.start, range.end)))
          .toList(growable: false),
...
List<JourneyDifficulty> supportedJourneyDifficulties(DailyJourneyExperience experience) {
  if (experience.id == 'beijing-summer-palace') return JourneyDifficulty.values;
...
JourneyLevelContent _withVocabularyInContext(JourneyLevelContent content) {
  ...
  return JourneyLevelContent(
    ...
    words: content.words.where((entry) => visibleContext.contains(entry.word)).toList(growable: false),
...
List<WordEntry> _selectSummerPalaceWords(List<String> selectedWords) =>
    summerPalaceWords.where((entry) => selectedWords.contains(entry.word)).toList(growable: false);
...
final summerPalaceEasyLevel = JourneyLevelContent(
  ...
  storyAnnotations: const <ReadingAnnotation>[
    ReadingAnnotation(
      pinyin: 'Shíqī suì de xuéshēng shèyǐngzhě Xǔ Chéng yào wèi xiàozhǎn pāi yì zhāng Yíhéyuán zuòpǐn, zhèngmíng zìjǐ bù xūyào wàipó Zhōu Lán zhǐdǎo. Zhōu Lán céng bǎohù Chángláng cǎihuà, tā yào Xǔ Chéng kàn xiūfù hénjì, Xǔ Chéng què zhǐ xiǎng pāi wúxiá huàmiàn.',
      vietnamese: 'Hứa Trừng, một nữ sinh nhiếp ảnh mười bảy tuổi, phải chụp tác phẩm Di Hòa Viên cho triển lãm trường để chứng minh không cần bà ngoại Chu Lam hướng dẫn. Chu Lam từng bảo tồn tranh màu Trường Lang; bà muốn cô nhìn dấu vết phục hồi, còn cô chỉ muốn khung hình không tì vết.',
      english: 'Seventeen-year-old student photographer Xu Cheng must make a Summer Palace work for the school exhibition to prove she needs no guidance from Zhou Lan. Zhou Lan once conserved Long Corridor paintings and asks her to see restoration traces, while Xu Cheng wants a flawless image.',
    ),
    ReadingAnnotation(

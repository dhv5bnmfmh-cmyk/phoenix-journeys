...
final guangzhouChenClanGoldSemanticFingerprint = JourneySemanticFingerprint(
  ...
  mechanisms: const <NarrativeSemanticDimension, NarrativeMechanismFamily>{
    ...
    NarrativeSemanticDimension.protagonistRolePattern:
        NarrativeMechanismFamily.estrangedBirthMotherSeekingContinuedContact,
    NarrativeSemanticDimension.relationshipGeometry:
        NarrativeMechanismFamily.birthMotherAdultDaughterWithRemoteKinPressure,
    NarrativeSemanticDimension.goalMechanism:
        NarrativeMechanismFamily.continueContactWithoutErasingPresentIdentity,
    NarrativeSemanticDimension.conflictMechanism:
    ...
    NarrativeSemanticDimension.culturalAnchorFunction: NarrativeMechanismFamily
        .pooledLineageInstitutionIntensifiesSurnameClaim,
    NarrativeSemanticDimension.artifactObjectNarrativeFunction:
        NarrativeMechanismFamily.phoneAsRefusedPublicProof,
    NarrativeSemanticDimension.movementSpatialMechanism:
        NarrativeMechanismFamily.courtyardProgressionUnderRemoteFamilyGaze,
    NarrativeSemanticDimension.temporalPressureMechanism:
        NarrativeMechanismFamily.unplannedLiveFamilyCall,
    NarrativeSemanticDimension.supportingCharacterFunction:
        NarrativeMechanismFamily.daughterSetsBoundaryAndOffersSmallContinuation,
    NarrativeSemanticDimension.dramaticEngineFamily:
...
final suzhouGardenGoldSemanticFingerprint = JourneySemanticFingerprint(
  ...
  mechanisms: const <NarrativeSemanticDimension, NarrativeMechanismFamily>{
    ...
    NarrativeSemanticDimension.protagonistRolePattern:
        NarrativeMechanismFamily.dailyCaregiverReleasingVisualControl,
    NarrativeSemanticDimension.relationshipGeometry:
        NarrativeMechanismFamily.caregiverChildReciprocalAdjustment,
    NarrativeSemanticDimension.goalMechanism:
        NarrativeMechanismFamily.practiceIndependentMovementWithoutBreakingCare,
    NarrativeSemanticDimension.conflictMechanism:
    ...
    NarrativeSemanticDimension.artifactObjectNarrativeFunction:
        NarrativeMechanismFamily.raisedThenLoweredHandEmbodiesRestraint,
    NarrativeSemanticDimension.movementSpatialMechanism:
        NarrativeMechanismFamily.repeatedOcclusionAndReappearance,
    NarrativeSemanticDimension.temporalPressureMechanism:
        NarrativeMechanismFamily.lastWalkBeforeIndependentRoutine,
    NarrativeSemanticDimension.supportingCharacterFunction:
        NarrativeMechanismFamily.childAcceptsResponsibilityToWaitAndLookBack,
    NarrativeSemanticDimension.dramaticEngineFamily:
...
final longmenGoldSemanticFingerprint = JourneySemanticFingerprint(
  ...
  coreEvidence: List<NarrativeMechanismEvidence>.unmodifiable([
    _activeEvidence(
      ...
      NarrativeMechanismFamily.sharedCreditBeginsWithEvidencePledge,
      ['约定并列署名，共同为复原依据负责。'],
      'The work begins with a reciprocal credit-and-evidence pledge, so accountability is relational before the disputed layer exists.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.relationshipGeometry,
      NarrativeMechanismFamily.equalCollaboratorsWithMutualCreditBoundary,
      ['没依据，不能放在我们两人的名字下。', '导出前，周澄保留并列署名'],
      'Zhou Cheng can withhold shared credit from unsupported work, while the final preserved credit depends on Lin Yan honoring their equal-author agreement.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.conflictMechanism,
      NarrativeMechanismFamily.aestheticCompletionVsJointAccountability,
      ['林砚为转场做了“补全脸部”模型。', '没依据，不能放在我们两人的名字下。'],
      'The desired seamless image now risks attributing an unsupported claim to both collaborators.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.choiceMechanism,
      NarrativeMechanismFamily.destructiveRemovalToPreserveJointAccountability,
      ['亲手关掉那层。'],
      'Lin Yan destroys usable completed work so the shared artifact can remain jointly accountable.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.climaxMechanism,
      NarrativeMechanismFamily.coauthorCreditBoundaryForcesDeletion,
      ['她看了三天渲染和片尾两个名字，亲手关掉那层。'],
      'The co-authorship boundary is physically visible at the moment she deletes the unsupported layer.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.consequenceMechanism,
      NarrativeMechanismFamily.sourcedTimelineReplacesUnsupportedClimax,
      ['短片失去最顺滑的镜头；周澄把老照片接回时间线。'],
      'Deletion visibly removes the smooth climax and causes the sourced evidence sequence to return to the edit.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.transformationMechanism,
      NarrativeMechanismFamily.soloVisualJudgmentToPrecommittedSourceReview,
      ['两人的合作也变了：林砚开始在建模前先问，每一层证据究竟能支持到哪里。'],
      'Her future behavior moves source review before modeling and makes Zhou Cheng part of authorship rather than a final checker.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.endingMechanism,
      NarrativeMechanismFamily.mutualCreditAfterJointVerification,
      ['林砚核完，把那层改名为“无依据，不使用”，按下导出。'],
      'The film is exported only after joint evidence work is checked and the unsupported layer remains named but unused.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.culturalAnchorFunction,
      ...
      'Longmen-specific inscription, present loss, and historical photographs define different representational limits and create the dispute.',
    ),
    _activeEvidence(
      _longmen,
      NarrativeSemanticDimension.dramaticEngineFamily,
      NarrativeMechanismFamily.evidenceBoundaryNegotiationPreservesCoauthorship,
      ['没依据，不能放在我们两人的名字下。', '亲手关掉那层。', '周澄保留并列署名'],
      'An equal collaborator invokes shared-liability boundaries; destructive deletion then preserves both truthful representation and co-authorship.',
    ),
  ]),
);
...
final kaipingGoldCandidateSemanticFingerprint = JourneySemanticFingerprint(
  ...
  coreEvidence: List<NarrativeMechanismEvidence>.unmodifiable([
    _activeEvidence(
      ...
      ['这份投入只用于家里的独楼；若要改作别用，就寄还给他。'],
      'The overseas brother opens the Story with a conditional family contribution that makes later reallocation relationally costly.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.relationshipGeometry,
      NarrativeMechanismFamily.separatedSiblingsRenegotiateSharedResponsibility,
      ['梁川把改过的众楼图和退还投入的承诺一起寄给哥哥。', '改写成“我们家在众楼里的一份”'],
      'The brothers act across distance through proposal, risk of return, and a written counter-commitment that changes ownership language.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.conflictMechanism,
      NarrativeMechanismFamily.privateCommemorationVsCommunalSafety,
      ['这份投入只用于家里的独楼', '村里几户人家正合建一座众楼，遇到危险时可临时避难。'],
      'A private family monument and its condition compete with the village need for a jointly used refuge.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.choiceMechanism,
      NarrativeMechanismFamily.riskReturnToRequestCollectiveConsent,
      ['家里放弃独建；梁川把改过的众楼图和退还投入的承诺一起寄给哥哥。'],
      'Liang Chuan enacts the choice while accepting that his brother may withdraw the contribution.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.climaxMechanism,
      NarrativeMechanismFamily.replyRewritesPrivateOwnershipAsSharedPart,
      ['改写成“我们家在众楼里的一份”'],
      'Liang Hai’s written revision converts the disputed private tower into a bounded family share of collective responsibility.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.consequenceMechanism,
      NarrativeMechanismFamily
          .communalConstructionContinuesWithFamilyCommitment,
      ['众楼继续施工；原图留在箱底。'],
      'The reply directly permits construction to continue while the private design is materially set aside.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.transformationMechanism,
      NarrativeMechanismFamily.inheritedCopyingModelToLocalRecombination,
      ['只把拱券留在入口，没有让柱廊占掉多户避难的位置。', '梁海下一封信问的不是“像不像原图”'],
      'Both brothers move from fidelity to the returned sketch toward selective form and accountable communal use.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.endingMechanism,
      NarrativeMechanismFamily.fadedRevisionBesideUnbuiltPrivatePlan,
      ['纸上的柱廊没有盖成，入口那道拱券却通向多户共同使用的空间；梁海改过的那行字已经褪色。'],
      'The ending holds absent private form, surviving selected form, shared use, and the brother’s faded revision in one restrained image.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.culturalAnchorFunction,
      NarrativeMechanismFamily.communalTowerFunctionReorganizesImportedForm,
      ['外面的样子可以带回来，里面得让大家一起用。', '只把拱券留在入口，没有让柱廊占掉多户避难的位置。'],
      'The Kaiping distinction between imported architectural influence and communal-tower use directly reorganizes the building plan.',
    ),
    _activeEvidence(
      _kaiping,
      NarrativeSemanticDimension.dramaticEngineFamily,
      NarrativeMechanismFamily
          .transnationalLetterExchangeReallocatesFamilyBuildingDuty,
      ['若要改作别用，就寄还给他。', '梁川把改过的众楼图和退还投入的承诺一起寄给哥哥。', '改写成“我们家在众楼里的一份”'],
      'A transnational sibling exchange changes who the family contribution serves and makes continued communal building the visible result.',
    ),
  ]),
);

final _summerPalaceBaselineFingerprint = _convertBaselineFingerprint(
  baseline.approvedGoldSemanticFingerprints[_summerPalace]!,
);
...
final datongYungangGoldSemanticFingerprint = JourneySemanticFingerprint(
  ...
  coreEvidence: List<NarrativeMechanismEvidence>.unmodifiable([
    _activeEvidence(
      ...
      NarrativeSemanticDimension.openingMechanism,
      NarrativeMechanismFamily.capitalShiftChangesInheritedScale,
      ['北魏迁都洛阳后，云冈不再像从前那样营造巨像。'],
      'The verified capital-and-scale transition opens the fictional family pressure.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.relationshipGeometry,
      NarrativeMechanismFamily.designatedDaughterVsFatherAndEmergingPeers,
      ['父亲只肯把手艺交给一个人；留下的弟弟魏朔和同伴阿砾便要继续替她扶绳，或者散伙。'],
      'A father-daughter succession demand materially fixes the brother and peer beneath one authority.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.conflictMechanism,
      NarrativeMechanismFamily.exclusiveSuccessionVsDistributedResponsibility,
      ['魏岚想留下三个人，却不能同时保住“唯一传人”的位置。'],
      'The human conflict is exclusive recognition versus distributed agency.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.choiceMechanism,
      NarrativeMechanismFamily.cutSingleInheritedToolIntoThree,
      ['魏岚却把墨绳按在石阶上，割成三段，一段给魏朔，一段给阿砾，最后一段留在自己掌心。'],
      'She enacts the choice through an irreversible division of the authority-bearing tool.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.climaxMechanism,
      NarrativeMechanismFamily.waitingFatherWitnessesIrreversibleCut,
      ['天亮时，父亲伸手等她收绳。魏岚却把墨绳按在石阶上，割成三段'],
      'The cut happens under the waiting father’s gaze rather than at task completion.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.consequenceMechanism,
      NarrativeMechanismFamily.threeIndependentMarksReplaceSingleCommand,
      ['三个人不再等同一双手发号施令。魏朔第一次自己弹出墨线'],
      'The choice visibly changes who may place the next line.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.transformationMechanism,
      NarrativeMechanismFamily.chosenHeirToEqualResponsibleMaker,
      ['她失去了父亲许诺的唯一位置。', '三个人不再等同一双手发号施令。'],
      'Status loss creates equal responsibility rather than mentor approval.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.endingMechanism,
      NarrativeMechanismFamily.fatherLooksBackWithoutReconciliation,
      ['父亲走到崖路转弯处，回头看了那三道黑线一眼。魏岚没有追上去。'],
      'A look and non-pursuit close the changed relationship without explanation.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.culturalAnchorFunction,
      ...
      ['黑印落在一块只够开小龛的石面上。'],
      'The smaller post-capital carving scale carries the consequence into place.',
    ),
    _activeEvidence(
      _datong,
      NarrativeSemanticDimension.dramaticEngineFamily,
      NarrativeMechanismFamily.scaleTransitionForcesInheritanceRedistribution,
      ['云冈不再像从前那样营造巨像。', '魏岚却把墨绳按在石阶上，割成三段'],
      'A verified shift of scale forces a fictional redistribution of inherited authority.',
    ),
  ]),
);
...
String activeCanonicalGoldStoryText(String journeyId) {
  ...
  if (journeyId == _longmen) {
    return List<String>.generate(
      10,
      (index) =>
          luoyangLongmenGoldLevelContent(index + 1).storyParagraphs.join('\n'),
    ).join('\n');
  }
  if (journeyId == _kaiping) {
    return List<String>.generate(
      10,
      (index) =>
          kaipingDiaolouGoldLevelContent(index + 1).storyParagraphs.join('\n'),
    ).join('\n');
  }
  if (journeyId == _datong) {
    return List<String>.generate(
      10,
      (index) =>
          datongYungangGoldLevelContent(index + 1).storyParagraphs.join('\n'),
    ).join('\n');
  }
  if (journeyId == _lijiang) {
    return List<String>.generate(
      10,
      (index) =>
          lijiangOldTownGoldLevelContent(index + 1).storyParagraphs.join('\n'),
    ).join('\n');
  }
...
List<String> semanticFingerprintCompletenessErrors(
  ...
  for (final dimension in narrativeSemanticCoreDimensions) {
    ...
    if (records.isEmpty) {
      errors.add(
        '${fingerprint.journeyId}: missing evidence ${dimension.name}',
      );
      continue;
    }
    if (records.length != 1) {
      errors.add(
        '${fingerprint.journeyId}: duplicate evidence ${dimension.name}',
      );
      continue;
    }
    if (records.single.mechanism != fingerprint.mechanism(dimension)) {
      errors.add(
        '${fingerprint.journeyId}: evidence mismatch ${dimension.name}',
      );
    }
  }
  return errors;

...
final longmenGoldSemanticFingerprint = JourneySemanticFingerprint(
  ...
  coreEvidence: List<NarrativeMechanismEvidence>.unmodifiable([
    _activeEvidence(_longmen, NarrativeSemanticDimension.openingMechanism, NarrativeMechanismFamily.sharedCreditBeginsWithEvidencePledge, ['约定并列署名，共同为复原依据负责。'], 'The work begins with a reciprocal credit-and-evidence pledge, so accountability is relational before the disputed layer exists.'),
    _activeEvidence(_longmen, NarrativeSemanticDimension.relationshipGeometry, NarrativeMechanismFamily.equalCollaboratorsWithMutualCreditBoundary, ['没依据，不能放在我们两人的名字下。', '导出前，周澄保留并列署名'], 'Zhou Cheng can withhold shared credit from unsupported work, while the final preserved credit depends on Lin Yan honoring their equal-author agreement.'),
    _activeEvidence(_longmen, NarrativeSemanticDimension.conflictMechanism, NarrativeMechanismFamily.aestheticCompletionVsJointAccountability, ['林砚为转场做了“补全脸部”模型。', '没依据，不能放在我们两人的名字下。'], 'The desired seamless image now risks attributing an unsupported claim to both collaborators.'),
    _activeEvidence(_longmen, NarrativeSemanticDimension.choiceMechanism, NarrativeMechanismFamily.destructiveRemovalToPreserveJointAccountability, ['亲手关掉那层。'], 'Lin Yan destroys usable completed work so the shared artifact can remain jointly accountable.'),
    _activeEvidence(_longmen, NarrativeSemanticDimension.climaxMechanism, NarrativeMechanismFamily.coauthorCreditBoundaryForcesDeletion, ['她看了三天渲染和片尾两个名字，亲手关掉那层。'], 'The co-authorship boundary is physically visible at the moment she deletes the unsupported layer.'),
    _activeEvidence(_longmen, NarrativeSemanticDimension.consequenceMechanism, NarrativeMechanismFamily.sourcedTimelineReplacesUnsupportedClimax, ['短片失去最顺滑的镜头；周澄把老照片接回时间线。'], 'Deletion visibly removes the smooth climax and causes the sourced evidence sequence to return to the edit.'),
    ...
    _activeEvidence(_longmen, NarrativeSemanticDimension.endingMechanism, NarrativeMechanismFamily.mutualCreditAfterJointVerification, ['林砚核完，把那层改名为“无依据，不使用”，按下导出。'], 'The film is exported only after joint evidence work is checked and the unsupported layer remains named but unused.'),
...
final summerPalaceCulturalIntegrationSemanticFingerprint = JourneySemanticFingerprint(
  ...
  coreEvidence: List<NarrativeMechanismEvidence>.unmodifiable([
    ...
    NarrativeMechanismEvidence(journeyId: _summerPalace, dimension: NarrativeSemanticDimension.conflictMechanism, mechanism: _summerPalaceBaselineFingerprint.mechanism(NarrativeSemanticDimension.conflictMechanism), activeSourceId: activeGoldStorySourceId, sourceTexts: const ['许澄避开旧痕迹，周岚要她多看一眼。', '旧照片背面写着：“一八六〇年受损，一八八六年开始修复。”许澄想把这行字裁掉，周岚把照片翻回正面。'], semanticRationale: 'Aesthetic perfection collides physically with restoration history through the old photograph.'),
    ...
    NarrativeMechanismEvidence(journeyId: _summerPalace, dimension: NarrativeSemanticDimension.culturalAnchorFunction, mechanism: _summerPalaceBaselineFingerprint.mechanism(NarrativeSemanticDimension.culturalAnchorFunction), activeSourceId: activeGoldStorySourceId, sourceTexts: const ['周岚做过园林修复，带着旧照片。', '旧照片背面写着：“一八六〇年受损，一八八六年开始修复。”许澄想把这行字裁掉，周岚把照片翻回正面。'], semanticRationale: 'Verified damage/restoration history is attached to the core object and causes framing conflict.'),

...
class _ShadowingTrainingScreenState extends State<ShadowingTrainingScreen> {
  ...
  Future<void> _initialize() async {
    ...
    await prefs.setString('phoenix.shadowing.dailyPlan', _dailyPlan!.encode());
  ...
  Future<void> _scoreCurrentAttempt({double confidence = 0}) async {
    ...
    if (dailyPlan != null) {
      await prefs.setString('phoenix.shadowing.dailyPlan', dailyPlan.encode());
    }
    if (!mounted) return;
    setState(() {
      _attempts = attempts;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: PhoenixTheme.gold.withValues(alpha: .62)),
          ),
          child: Column(
            children: [
              Row(

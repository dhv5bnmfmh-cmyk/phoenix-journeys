...
void main() {
  test('all non-Datong approved Gold Journeys own a complete Challenge profile', () {
    ...
    for (final profile in nonDatongGoldChallengeProfiles.values) {
      ...
      expect(profile.storyDistractors.length, greaterThanOrEqualTo(6),
          reason: profile.journeyId);

      for (var index = 1; index < 10; index++) {
        expect(profile.paragraphAnchors[index],
            greaterThan(profile.paragraphAnchors[index - 1]),
            reason: '${profile.journeyId} paragraph Lv${index + 1}');
        expect(profile.missingAnchors[index],
            greaterThan(profile.missingAnchors[index - 1]),
            reason: '${profile.journeyId} missing Lv${index + 1}');
      }

      final distractorTexts =
          profile.storyDistractors.map((item) => item.text).toList();
      expect(distractorTexts.toSet().length, distractorTexts.length,
      ...
      for (var level = 1; level <= 10; level++) {
        ...
        expect(grammar.brokenSentence, isNot(grammar.correctedSentence),
            reason: '${profile.journeyId} Lv$level no-op grammar');
        expect(grammar.brokenSegment, isNot(grammar.correctReplacement),
            reason: '${profile.journeyId} Lv$level no-op replacement');
        expect(grammar.distractors.length, 3,
  ...
  test('Journey-specific prompts and misconception sets do not collide exactly', () {
    final paragraphPrompts = <String>{};
    final missingPrompts = <String>{};
    final allDistractors = <String>{};

    for (final profile in nonDatongGoldChallengeProfiles.values) {
      expect(paragraphPrompts.add(profile.paragraphPrompt), isTrue,
          reason: '${profile.journeyId} paragraph prompt collision');
      expect(missingPrompts.add(profile.missingPrompt), isTrue,
          reason: '${profile.journeyId} missing prompt collision');
      for (final item in profile.storyDistractors) {
        expect(allDistractors.add(item.text), isTrue,

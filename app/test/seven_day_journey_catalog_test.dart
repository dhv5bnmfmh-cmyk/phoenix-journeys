...
void main() {
  test('reviewed destinations rotate without repeating during one cycle', () {
    expect(dailyJourneyExperiences, hasLength(34));
    expect(dailyJourneyExperiences.map((item) => item.id).toSet(), hasLength(dailyJourneyExperiences.length));
    final cycle = List.generate(dailyJourneyExperiences.length, (index) => dailyJourneyForDate(DateTime(2026, 1, 1 + index)).id);
    ...
    for (final journey in dailyJourneyExperiences) {
      if (journey.id == 'beijing-summer-palace') {
        ...
        for (final paragraph in journey.content.storyParagraphs) {
          expect(paragraph.length, inInclusiveRange(260, 380), reason: journey.id);
        }
      } else {
        expect(journey.content.storyParagraphs.length, 4, reason: journey.id);
      }
      expect(journey.storyAnnotations.length, journey.content.storyParagraphs.length, reason: journey.id);

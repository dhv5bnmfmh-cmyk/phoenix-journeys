...
void main() {
  test(
    'municipalities resolve one node as province-level and city-equivalent',
    () {
  ...
  test(
    'China prefecture and county levels project without rewriting GeoNodes',
    () {
      ...
      expect(honghe.compactAdministrativeLabel, '云南省 · 红河哈尼族彝族自治州');
    },
  );
  ...
  test('every registered production Journey has display-ready geography', () {
    ...
    for (final location in geographicJourneys) {
      ...
      expect(location.provinceLevelNode, isNotNull, reason: location.journeyId);
      expect(
        location.cityEquivalentNode,
        isNotNull,

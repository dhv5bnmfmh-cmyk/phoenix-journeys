...
void main() {
  ...
  test(
    ...
    expect(
      forbiddenCityStoryCatalog.map((story) => story.title),
      <String>['两条路，一张图', '交接前的标记'],
    );
    expect(
  ...
  test(
      'Second Story runtime imports approved pipeline content without regeneration',
      () {
  ...
  test('Second Story Challenge consumes only approved taught target material',
      () {
  ...
  test('Story progress, completion, resume and Memory are isolated by Story ID',
      () async {
    ...
    final secondProgress =
        await state.forbiddenCityStoryProgress(forbiddenCitySecondStoryId);

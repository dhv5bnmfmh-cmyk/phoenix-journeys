...
| NJ-053 | Source Truth Gate | REQUIRED | Authoritative source inventory plus verified world premises; material factual world claims trace to approved source IDs while fictional ordinary-character private material is not documentary-source-gated |  | BLOCKED | UNVERIFIED | `SOURCE EVIDENCE INSUFFICIENT — STORY DEVELOPMENT STOPPED` only when a required world fact is unavailable or all plausible approaches require invented history |  | NOT_REQUIRED |
| NJ-054 | Fact / Fiction Classification | REQUIRED | Every material premise classified by WORLD CLAIM / CHARACTER FICTION / INTERPRETIVE DEVICE and the canonical verified, fictional, legend/contested/unknown, or unsupported category |  | BLOCKED | UNVERIFIED | Fictional private action requires plausibility, not archival proof; `UNVERIFIED FACTUAL CLAIM — BLOCKED` remains binding for unsupported world claims |  | NOT_REQUIRED |
...
| NJ-057 | Generic Place Substitution Test | REQUIRED | Human-auditable control removes/replaces the verified place-specific cause without transplanting it, then tests whether the current Goal→Conflict→Choice→Climax→Consequence survives for substantially the same reason without substantial redesign |  | BLOCKED | UNVERIFIED | `GENERIC-PLACE STORY — NOT GOLD READY` when substantially interchangeable; do not move the cause into the control |  | NOT_REQUIRED |
...
| NJ-065A | Fictional ordinary-character freedom | REQUIRED | Clearly fictional identity/backstory/relationship/private action/dialogue/motivation/choice/cost/consequence assessed for plausibility and non-contradiction, not documentary existence |  | BLOCKED | UNVERIFIED | `FICTIONAL CHARACTER ACTION` is not an `UNSUPPORTED FACTUAL CLAIM`; lacking a source for private fiction does not reclassify it as unsupported history |  | NOT_REQUIRED |
| NJ-065B | Evidence-blocked disposition threshold | REQUIRED | Record distinguishes failed Story architecture (`REDESIGN`) from missing required world evidence (`SOURCE EVIDENCE INSUFFICIENT`) |  | BLOCKED | UNVERIFIED | Several failed plots do not by themselves evidence-block a Journey |  | NOT_REQUIRED |
...
| NJ-116 | Fact / interpretation / fiction boundary | REQUIRED | Verified world fact/action/practice/condition, fictional identity/backstory/action/relationship/dialogue/motivation/choice/cost/consequence, interpretive narration, legend, contested interpretation, and unknown remain separately classified |  | BLOCKED | UNVERIFIED | Narrative convenience cannot upgrade truth status; fictional private life does not require documentary proof |  | NOT_REQUIRED |

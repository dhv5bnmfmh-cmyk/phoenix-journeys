...
test('preview map keeps the Beijing Reference publication boundary', () => {
  assert.match(passport, /earned \|\| allAccess[\s\S]*journey\.stampSymbol/);
  assert.match(passport, /publishedJourneyRuntimeIds/);
  assert.match(passport, /'北京 · 紫禁城 · 两条路，一张图'/);
  assert.match(passport, /'北京 Reference'/);
  assert.doesNotMatch(passport, /'体验版已开放全部城市与学习步骤。'/);
});

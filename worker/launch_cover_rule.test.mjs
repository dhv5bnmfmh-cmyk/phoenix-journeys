...
test('Phoenix launch cover animates a journey from ancient to modern worlds', async () => {
  ...
  assert.match(bootstrap, /phoenixFlightAnimationName = 'phoenix-time-flight-v2'/);
  assert.match(bootstrap, /animation\.animationName === phoenixFlightAnimationName/);
  assert.match(bootstrap, /const phoenixFlightFinished = waitForPhoenixFlight\(\)/);
  assert.match(bootstrap, /const \[runAppReadyAt, flight\] = await Promise\.all\(\[/);
  assert.match(bootstrap, /appRunner\.runApp\(\)\.then/);
  assert.match(bootstrap, /phoenixFlightFinished/);

...
const flightMap =
  'app/assets/images/home/east-asia-flight-map-hd.webp';
...
test('welcome flight card uses a bundled layered HD map', () => {
  assert.equal(existsSync(flightMap), true);
  assert.ok(statSync(flightMap).size > 100_000);
  assert.ok(statSync(flightMap).size < 400_000);
  assert.match(explore, /east-asia-flight-map-hd\.webp/);
  assert.match(explore, /phoenix-flight-map-hd-image/);
  assert.match(explore, /precacheImage\(const AssetImage\(_phoenixFlightMapAsset\)/);
  assert.doesNotMatch(explore, /void _draw(?:Grid|Stars|Land|ComicDetails)/);
});

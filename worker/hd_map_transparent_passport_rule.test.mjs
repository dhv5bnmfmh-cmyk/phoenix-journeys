...
test('special journeys open from a featured wallet-aware button', () => {
  ...
  assert.match(special, /height: 52/);
  ...
  assert.match(special, /state\.silverFragments/);
  assert.match(special, /SpecialJourneyCurrencyLegend\(/);
  assert.match(special, /余额充足/);
  assert.match(special, /还差 \$missing 枚/);

...
test('Flutter-backed runner requires forty-one journeys and 410 inspections', () => {
  ...
  assert.match(reportRunner, /report\['journeyCount'\], 41/);
  assert.match(reportRunner, /report\['regularJourneyCount'\], 32/);
  assert.match(reportRunner, /report\['specialJourneyCount'\], 9/);
  assert.match(reportRunner, /report\['inspectionCount'\], 410/);
